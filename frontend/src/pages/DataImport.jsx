...
export default function DataImport() {
  ...
  return <div className="container mx-auto p-6 space-y-6"><PageHeader title="Data Import" description="Upload XMI, XML, CSV, or JSON files to import into the knowledge graph" icon={<Upload className="h-6 w-6 text-primary" />} breadcrumbs={[{
      ...
      label: 'Data Import'
          ...
          })}</div></CardContent></Card>{allJobs.length > 0 && <Card className="card-corporate border-2"><CardHeader className="border-b bg-linear-to-r from-accent/10 to-accent/5"><div className="flex items-center justify-between"><CardTitle>Upload History</CardTitle><Button variant="outline" size="sm" onClick={() => refetch()}><RefreshCw className="h-4 w-4 mr-2" />Refresh</Button></div></CardHeader><CardContent className="pt-6"><div className="rounded-lg border-2 overflow-hidden"><Table><TableHeader className="bg-muted/50"><TableRow><TableHead>Status</TableHead><TableHead>Filename</TableHead><TableHead>Progress</TableHead><TableHead>Message</TableHead><TableHead className="w-[100px]">Actions</TableHead></TableRow></TableHeader><TableBody>{allJobs.map(job => <TableRow key={job.job_id}><TableCell><div className="flex items-center gap-2">{getStatusIcon(job.status)}{getStatusBadge(job.status)}</div></TableCell><TableCell className="font-medium"><div className="flex items-center gap-2"><File className="h-4 w-4 text-muted-foreground" />{job.filename}</div></TableCell><TableCell><div className="space-y-2 min-w-[200px]"><Progress value={job.progress} className="h-2" /><span className="text-xs text-muted-foreground">{job.progress}%</span></div></TableCell><TableCell className="max-w-md"><div className="space-y-1">{job.message && <p className="text-sm">{job.message}</p>}{job.error && <p className="text-sm text-red-500 flex items-center gap-1"><AlertCircle className="h-4 w-4" />{job.error}</p>}{job.stats && job.status === 'completed' && <div className="text-xs text-muted-foreground">{JSON.stringify(job.stats).slice(0, 100)}...</div>}</div></TableCell><TableCell><Button variant="ghost" size="sm" onClick={() => {
                    setUploadedJobs(prev => prev.filter(j => j.job_id !== job.job_id));
                  }}><Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive" /></Button></TableCell></TableRow>)}</TableBody></Table></div></CardContent></Card>}</div>;
}
