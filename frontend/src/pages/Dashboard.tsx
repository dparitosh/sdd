...
import { Database, GitBranch, Package, Activity, TrendingUp, Search, Terminal, FileText, ArrowRight, Sparkles } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const nodeTypes = Object.entries(stats?.node_types || {}).sort(([, a], [, b]) => (b as number) - (a as number));
  const relationshipTypes = Object.entries(stats?.relationship_types || {}).sort(([, a], [, b]) => (b as number) - (a as number));
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-background border-2 border-primary/20 shadow-lg p-8">
        <div className="absolute top-0 right-0 -mt-8 -mr-8 h-40 w-40 rounded-full bg-primary/10 blur-3xl" />
        <div className="absolute bottom-0 left-0 -mb-8 -ml-8 h-40 w-40 rounded-full bg-primary/10 blur-3xl" />
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-4xl font-bold tracking-tight">Knowledge Graph Dashboard</h1>
                <Badge className="bg-green-500 text-white">Online</Badge>
              </div>
              <p className="text-lg text-muted-foreground">Periodic table view of system components</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-2">
        {gridItems.map((item, index) => (
          ...
          >
            <div className={`absolute inset-0 bg-gradient-to-br ${item.color} opacity-8 group-hover:opacity-15 transition-opacity`} />
            <CardContent className="p-3 relative h-full flex flex-col items-center justify-center min-h-[85px]">
              <div className="text-xs text-muted-foreground mb-2 truncate w-full text-center leading-tight font-medium">{item.name}</div>
              <div className="text-2xl font-bold truncate w-full text-center">{item.value.toLocaleString()}</div>
              <Badge variant="outline" className="text-[8px] px-1 py-0 mt-1.5 leading-none">
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <div className="space-y-1">
              ...
              <p className="font-semibold">Neo4j Aura</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Protocol</p>
              <p className="font-semibold">ISO 10303 SMRL</p>
            </div>
            <div className="space-y-1">
