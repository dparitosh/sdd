...
export default function ModelChat() {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const result = await apiService.agents.runOrchestrator(userMsg, 'knowledge_query');
  ...
  return <div className="container mx-auto p-6 h-[calc(100vh-8rem)] flex flex-col space-y-6"><PageHeader title="Model Chat" description="Conversational interface to explore your knowledge graph using natural language" icon={<Sparkles className="h-6 w-6 text-primary" />} badge="AI Beta" breadcrumbs={[{
      ...
      label: 'Model Chat'
    }]} /><Card className="flex-1 flex flex-col"><CardContent className="flex-1 flex flex-col gap-4 pt-6"><ScrollArea className="flex-1 pr-4"><div className="space-y-4">{messages.map((msg, idx) => <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>{msg.role === 'assistant' && <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center shrink-0"><Bot className="h-4 w-4 text-primary" /></div>}<div className={`max-w-[80%] p-4 rounded-lg ${msg.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}><p className="text-sm">{msg.content}</p></div>{msg.role === 'user' && <div className="h-8 w-8 rounded-full bg-primary flex items-center justify-center shrink-0"><User className="h-4 w-4 text-primary-foreground" /></div>}</div>)}</div></ScrollArea><div className="flex gap-2"><Input placeholder="Ask me anything about your knowledge graph..." value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} className="flex-1" disabled={sending} /><Button onClick={handleSend} disabled={sending}>{sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</Button></div><div className="flex gap-2 flex-wrap"><Badge variant="outline" className="cursor-pointer hover:bg-primary/10">Show all requirements</Badge><Badge variant="outline" className="cursor-pointer hover:bg-primary/10">Find unlinked components</Badge><Badge variant="outline" className="cursor-pointer hover:bg-primary/10">Simulation results summary</Badge></div></CardContent></Card></div>;
}
