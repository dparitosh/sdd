...
import { Download, RefreshCw, Search, CheckCircle2, AlertCircle, XCircle } from 'lucide-react';
...
export default function TraceabilityMatrix() {
    ...
    queryFn: async () => {
      ...
      for (const req of requirements.slice(0, 50)) { // Limit to first 50 for performance
        try {
          ...
          console.error(`Failed to fetch traceability for ${req.uid}:`, error);
        }
      }
  ...
  const groupedData = filteredData.reduce((acc, link) => {
    ...
    return acc;
  }, {} as Record<string, { requirement: any; targets: TraceabilityLink[] }>);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Visualize and analyze requirement traceability relationships
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <Alert>
              <AlertDescription>
                No traceability data found. Make sure requirements have relationships defined in the database.
              </AlertDescription>
            </Alert>
          ) : (
