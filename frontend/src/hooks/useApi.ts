...
import { getStatistics, executeCypher } from '@/services/query.service';
import { ap239, ap242, smrl } from '@/services/standards.service';
import { getGraphData } from '@/services/graph.service';
import { uploadFile, getUploadStatus } from '@/services/upload.service';
...
export function useRequirements(params?: { limit?: number; offset?: number }) {
  ...
  return useQuery({
    ...
    queryFn: () => ap239.getRequirements(params),
    staleTime: QUERY_CONFIG.STALE_TIME,
    onError: () => {
...
export function useRequirement(uid: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: () => uid ? ap239.getRequirement(uid) : Promise.reject(),
    enabled: !!uid,
    staleTime: QUERY_CONFIG.STALE_TIME,
    onError: () => {
...
export function useRequirementTraceability(uid: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: () => uid ? ap239.getRequirementTraceability(uid) : Promise.reject(),
    enabled: !!uid,
    staleTime: QUERY_CONFIG.STALE_TIME,
    onError: () => {
...
export function usePart(id: string | undefined) {
  ...
  return useQuery({
    queryKey: ['part', id],
    queryFn: () => id ? ap242.getPart(id) : Promise.reject(),
    enabled: !!id,
    staleTime: QUERY_CONFIG.STALE_TIME,
    onError: () => {
...
export function useCreateResource(type: string) {
  ...
  return useMutation({
    mutationFn: (data: any) => smrl.createResource(type, data),
    onSuccess: () => {
...
export function useUpdateResource(type: string) {
  ...
  return useMutation({
    mutationFn: ({ uid, data }: { uid: string; data: any }) =>
      smrl.updateResource(type, uid, data),
...
export function useDeleteResource(type: string) {
  ...
  return useMutation({
    mutationFn: (uid: string) => smrl.deleteResource(type, uid),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [type.toLowerCase() + 's'] });
      toast.success(t('common.success'), {
...
export function useUploadStatus(jobId: string | undefined, enabled = true) {
  ...
  return useQuery({
    ...
    queryFn: () => jobId ? getUploadStatus(jobId) : Promise.reject(),
...
export function useCypherQuery() {
  ...
  return useMutation({
    mutationFn: (query: string) => executeCypher(query),
    onError: (error: any) => {
      toast.error(t('errors.apiError', { 
