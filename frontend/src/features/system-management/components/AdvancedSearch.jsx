...
function SearchResultsGraph({ results }) {
  ...
  // Track container width via ResizeObserver so canvas always fills its parent
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (clientWidth > 0) {
        setDimensions({ width: clientWidth, height: Math.max(400, clientHeight) });
      }
    };
    measure();
  ...
  const graphData = useMemo(() => {
    ...
    // Link nodes that share the same type (chain within each type group)
    const links = [];
    const typeGroups = {};
    nodes.forEach(n => {
  ...
  // Freeze simulation + center on click so the graph stays static
  ...
  if (!results || results.length === 0) {
    return (
      <div className="flex items-center justify-center h-125 text-muted-foreground">
  ...
  return (
    <div ref={containerRef} className="relative w-full h-125 overflow-hidden">
      {dimensions.width > 0 && (
        <ForceGraph2D
          ref={fgRef}
          graphData={graphData}
          ...
          nodeCanvasObject={(node, ctx, globalScale) => {
            ...
            // Label (always visible — no zoom threshold needed at this scale)
      ...
      {selectedNode && (
        <div className="absolute top-3 left-3 bg-background border rounded-lg p-3 shadow-lg max-w-xs z-10">
          <div className="flex items-center gap-2 mb-2">
            <Badge style={{ backgroundColor: getNodeColor(selectedNode), color: '#fff' }}>{selectedNode.type}</Badge>
