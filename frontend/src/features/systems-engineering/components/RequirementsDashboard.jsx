...
import { ap239, requirements as requirementsApi } from '@/services/standards.service';
import { getTraceabilityMatrix } from '@/services/graph.service';
...
const RequirementsDashboard = () => {
  ...
  const {
    ...
    queryFn: () => {
      ...
      return ap239.getRequirements(params);
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ uid, data }) => requirementsApi.update(uid, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ap239-requirements'] });
      queryClient.invalidateQueries({ queryKey: ['ap239-statistics'] });
  ...
  const fetchRequirementDetail = async reqId => {
    try {
      const response = await ap239.getRequirement(reqId);
