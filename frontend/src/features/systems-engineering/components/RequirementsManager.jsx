...
import { requirements as requirementsApi } from '@/services/standards.service';
...
export default function RequirementsManager() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // /api/v1/Requirement returns {resources: [...], count: N}
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      uid,
      data
    }) => requirementsApi.update(uid, data),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['requirements']
      });
  ...
  const deleteMutation = useMutation({
    mutationFn: uid => requirementsApi.delete(uid),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['requirements']
      });
  ...
  const columns = [{
    ...
    cell: ({
      row
    }) => <div className="max-w-75 truncate font-medium">{row.getValue('name')}</div>
  }, {
    accessorKey: 'description',
    header: 'Description',
    cell: ({
      row
    }) => <div className="max-w-100 truncate text-muted-foreground">{row.getValue('description') || '—'}</div>
  }, {
    accessorKey: 'status',
    header: 'Status',
    cell: ({
