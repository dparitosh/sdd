...
export default function SystemMonitoring() {
  ...
  const metrics = metricsSummary ? {
    apiRequestRate: Math.round((metricsSummary.api?.requests_per_second ?? 0) * 60),
    p95Latency: metricsSummary.api?.avg_response_time_ms ?? 0,
    cacheHitRate: (metricsSummary.cache?.hit_rate ?? 0) * 100,
    activeConnections: typeof metricsSummary.database?.active_connections === 'number' ? metricsSummary.database.active_connections : 0,
    errorRate: (1 - (metricsSummary.api?.success_rate ?? 1)) * 100,
