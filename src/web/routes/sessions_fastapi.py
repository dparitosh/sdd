...
from src.web.middleware.jwt_middleware import get_current_user_from_request, require_role
...
@router.delete("/me/{session_id}", response_model=MessageResponse, response_class=Neo4jJSONResponse)
...
@router.delete("/me/all", response_model=MessageResponse, response_class=Neo4jJSONResponse)
async def revoke_all_my_sessions(current_user: dict = Depends(get_current_user_from_request)):
    """
...
async def get_session_statistics(current_user: dict = Depends(get_current_user_from_request)):
    """
...
async def get_user_sessions_admin(
    ...
    try:
        ...
        logger.info(f"Admin {current_user['username']} retrieved sessions for user: {username}")
...
async def revoke_all_user_sessions_admin(
    ...
    try:
        ...
        logger.warning(f"Admin {current_user['username']} revoked all sessions for user: {username}")
...
async def cleanup_expired_sessions(current_user: dict = Depends(get_current_user_from_request)):
    """
    ...
    try:
        ...
        logger.info(f"Admin {current_user['username']} cleaned up {cleaned} expired sessions")
