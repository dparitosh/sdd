...
async def health_check():
    ...
    try:
        ...
        return Neo4jJSONResponse(content=health, status_code=200)
...
# Register route modules (to be converted from Flask blueprints)
logger.info("=" * 60)
...
try:
    from src.web.routes.core import router as core_router
...
try:
    from src.web.routes.ap239 import router as ap239_router
    app.include_router(ap239_router, prefix="/api/ap239", tags=["AP239"])
    logger.info("✓ Registered AP239 routes")
except ImportError as e:
    logger.warning(f"AP239 routes not yet migrated: {e}")

try:
    from src.web.routes.ap242 import router as ap242_router
    app.include_router(ap242_router, prefix="/api/ap242", tags=["AP242"])
    logger.info("✓ Registered AP242 routes")
except ImportError as e:
...
try:
    from src.web.routes.plm_connectors import router as plm_router
    app.include_router(plm_router, prefix="/api/v1/plm", tags=["PLM"])
    logger.info("✓ Registered PLM routes")
except ImportError as e:
