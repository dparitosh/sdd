...
if (-not $SkipPythonInstall) {
    ...
    if (Test-Path $RequirementsFile) {
        ...
        # Install Phase 2 extras (AI agents, PLM integration, semantic web, simulation)
        $Phase2File = Join-Path $InstallDir "backend\requirements-phase2.txt"
        if ($IncludePhase2 -and (Test-Path $Phase2File)) {
            Write-Host ""
            Write-Host "Installing Phase 2 dependencies from $Phase2File..."
            & $VenvPython -m pip install -r $Phase2File
            if ($LASTEXITCODE -eq 0) {
                Write-Host "[OK] Phase 2 Python dependencies installed" -ForegroundColor Green
            } else {
                Write-Host "[WARN] Some Phase 2 dependencies may have failed to install" -ForegroundColor Yellow
            }
        } elseif ($IncludePhase2 -and -not (Test-Path $Phase2File)) {
            Write-Host "[WARN] Phase 2 requirements file not found: $Phase2File" -ForegroundColor Yellow
        }
