...
def _reload_via_engine(store_type: str = "neo4j") -> None:
    ...
    logger.info("\n🚀 Running additional ingestion scripts...")
    import subprocess
    
    scripts_to_run = [
        {
            "name": "AP243 Ontology",
            "cmd": [sys.executable, os.path.join(backend_path, "scripts", "ingest_ontology_rdf.py"), "--path", os.path.join(project_root, "smrlv12", "data", "domain_models", "mossec", "ap243_v1.owl")]
        },
        {
            "name": "Semantic Layer",
            "cmd": [sys.executable, os.path.join(backend_path, "scripts", "ingest_semantic_layer.py")]
        },
        {
            "name": "SDD Schema Migration",
            "cmd": [sys.executable, os.path.join(backend_path, "scripts", "run_sdd_schema_migration.py")]
        },
        {
            "name": "SDD Data",
            "cmd": [sys.executable, os.path.join(backend_path, "scripts", "ingest_sdd_data.py")]
        },
        {
            "name": "Sample Data",
            "cmd": [sys.executable, os.path.join(backend_path, "scripts", "create_sample_data.py")]
        }
    ]

    # Set environment to force UTF-8 encoding for Python subprocesses
    env = os.environ.copy()
    ...
    for script in scripts_to_run:
        ...
        try:
            subprocess.run(script["cmd"], capture_output=True, text=True, check=True, env=env, encoding="utf-8")
