...
try {
    ...
    if ($nodeTypes -and $nodeTypes.Count -gt 0) {
        ...
        Write-Host "      [WARN] Graph API responded but no node types found -- database appears empty" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "      ── DATA LOADING REQUIRED ─────────────────────────────────────────────" -ForegroundColor Yellow
        Write-Host "      Run these commands from the repository root to seed the graph:" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "        [1] Base domain model + OSLC vocabulary (required first):" -ForegroundColor Cyan
        Write-Host "              .\.venv\Scripts\python.exe scripts\reload_database.py" -ForegroundColor White
        Write-Host ""
        Write-Host "        [2] AP hierarchy indexes + AP239/AP242/AP243 sample nodes:" -ForegroundColor Cyan
        Write-Host "              .\.venv\Scripts\python.exe backend\scripts\schema_migrator.py" -ForegroundColor White
        Write-Host ""
        Write-Host "        [3] Requirements and traceability links:" -ForegroundColor Cyan
        Write-Host "              .\.venv\Scripts\python.exe backend\scripts\create_sample_data.py" -ForegroundColor White
        Write-Host ""
        Write-Host "        [4] SDD schema + dossier data:" -ForegroundColor Cyan
        Write-Host "              .\.venv\Scripts\python.exe backend\scripts\run_sdd_schema_migration.py" -ForegroundColor White
        Write-Host "              .\.venv\Scripts\python.exe backend\scripts\ingest_sdd_data.py" -ForegroundColor White
        Write-Host ""
        Write-Host "        [5] SimulationRun workflow (requires step 4 first):" -ForegroundColor Cyan
        Write-Host "              .\.venv\Scripts\python.exe backend\scripts\run_simulation_run_migration.py" -ForegroundColor White
        Write-Host ""
        Write-Host "      See INSTALL.md Sections 6.2 and 6.6 for full details." -ForegroundColor Yellow
