...
class APCompletenessValidator:
    ...
    def _test_indexes_exist(self):
        ...
        required_indexes = [
            'idx_requirement_id', 'idx_requirement_name',
            'idx_part_id', 'idx_part_name',
            'idx_material_name', 'idx_external_owl_class_name'
        ]
    ...
    def _test_ap239_nodes(self):
        ...
        for node_type in node_types:
            query = f"MATCH (n:{node_type}) WHERE n.ap_level = 1 RETURN count(n) AS count"
            result = self.neo4j.execute_query(query)
    ...
    def _test_ap242_nodes(self):
        ...
        for node_type in node_types:
            query = f"MATCH (n:{node_type}) WHERE n.ap_level = 2 RETURN count(n) AS count"
            result = self.neo4j.execute_query(query)
            count = result[0]['count'] if result else 0
    ...
    def _test_ap243_nodes(self):
        ...
        for node_type in node_types:
            query = f"MATCH (n:{node_type}) WHERE n.ap_level = 3 RETURN count(n) AS count"
            result = self.neo4j.execute_query(query)
            count = result[0]['count'] if result else 0
    ...
    def _test_ap239_internal_relationships(self):
        ...
        relationships = [
            'SATISFIES', 'VERIFIES', 'REFINES', 'APPROVES', 'ANALYZES'
        ]
    ...
    def _test_ap242_internal_relationships(self):
        ...
        relationships = [
            'HAS_GEOMETRY', 'USES_MATERIAL', 'ASSEMBLES_WITH', 'HAS_VERSION'
        ]
    ...
    def _test_cross_level_relationships(self):
        ...
        if results:
            for r in results:
                self._pass(f"Level {r['from_level']}→{r['to_level']}: {r['rel_type']} ({r['count']:,})")
        else:
    ...
    def _test_requirement_to_part_chains(self):
        ...
        if result and result[0]['count'] > 0:
            ...
            self._pass(f"Found {count:,} chains (path length: {min_hops}-{max_hops} hops)")
        else:
    ...
    def _test_end_to_end_traceability(self):
        ...
        if result and result[0]['count'] > 0:
            ...
            min_hops = result[0]['min_hops']
            max_hops = result[0]['max_hops']
            self._pass(f"Found {count:,} complete chains (path length: {min_hops}-{max_hops} hops)")
        else:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Validate AP239/AP242/AP243 knowledge graph')
