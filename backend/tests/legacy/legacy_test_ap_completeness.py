...
class APCompletenessValidator:
    ...
    def _test_ap239_nodes(self):
        ...
        for node_type in node_types:
            query = (
                f"MATCH (n:{node_type}) WHERE n.ap_level = 'AP239' RETURN count(n) AS count"
            )
            result = self.neo4j.execute_query(query)
    ...
    def _test_ap242_nodes(self):
        ...
        for node_type in node_types:
            query = (
                f"MATCH (n:{node_type}) WHERE n.ap_level = 'AP242' RETURN count(n) AS count"
            )
            result = self.neo4j.execute_query(query)
            count = result[0]["count"] if result else 0
    ...
    def _test_ap243_nodes(self):
        ...
        for node_type in node_types:
            query = (
                f"MATCH (n:{node_type}) WHERE n.ap_level = 'AP243' RETURN count(n) AS count"
            )
            result = self.neo4j.execute_query(query)
            count = result[0]["count"] if result else 0
    ...
    def _test_ap239_internal_relationships(self):
        ...
        for rel_type in relationships:
            ...
            WHERE n1.ap_level = 'AP239' AND n2.ap_level = 'AP239'
    ...
    def _test_ap242_internal_relationships(self):
        ...
        for rel_type in relationships:
            query = f"""
            MATCH (n1)-[r:{rel_type}]->(n2)
            WHERE n1.ap_level = 'AP242' AND n2.ap_level = 'AP242'
            RETURN count(r) AS count
            """
            result = self.neo4j.execute_query(query)
            count = result[0]["count"] if result else 0
    ...
    def _test_requirement_to_part_chains(self):
        ...
        WHERE req.ap_level = 'AP239' AND part.ap_level = 'AP242'
    ...
    def _test_requirement_to_ontology_chains(self):
        ...
        WHERE req.ap_level = 'AP239' AND owl.ap_level = 'AP243'
        RETURN count(DISTINCT path) AS count, min(length(path)) AS min_hops, max(length(path)) AS max_hops
        """
        result = self.neo4j.execute_query(query)
    ...
    def _test_ap_level_consistency(self):
        ...
        WHERE n.ap_level = 'AP239' AND n.ap_schema <> 'AP239'
           OR n.ap_level = 'AP242' AND n.ap_schema <> 'AP242'
           OR n.ap_level = 'AP243' AND n.ap_schema <> 'AP243'
