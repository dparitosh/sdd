    ...
    def test_endpoint(self, name: str, method: str, url: str, expected_status: int = 200, 
        ...
        try:
            ...
            if response.status_code == expected_status:
                print(f"{GREEN}✓{RESET} {name}: {response.status_code} (Expected: {expected_status})")
                self.passed += 1
                return response
            else:
                print(f"{RED}✗{RESET} {name}: {response.status_code} (Expected: {expected_status})")
...
def main():
    ...
    response = tester.test_endpoint(
        "Login (Expected Fail)", "POST", f"{BASE_URL}/auth/login",
        expected_status=401,
    ...
    tester.test_endpoint(
        "Verify Token (No Auth)", "GET", f"{BASE_URL}/auth/verify",
        expected_status=401
    )
    ...
    tester.test_endpoint(
        "PLM Parameters", "GET", f"{BASE_URL}/plm/parameters?limit=10"
    )
    ...
    tester.test_endpoint(
        "PLM Composition (404 expected)", "GET", f"{BASE_URL}/plm/composition/test-node",
        expected_status=404
    )
    
    tester.test_endpoint(
        "PLM Impact Analysis (404 expected)", "GET", f"{BASE_URL}/plm/impact/test-node",
        expected_status=404
    )
    ...
    tester.test_endpoint(
        "Simulation Units", "GET", f"{BASE_URL}/simulation/units"
    )
    
    # Test validation endpoint
    tester.test_endpoint(
        "Simulation Validate", "POST", f"{BASE_URL}/simulation/validate",
        data={
            "parameters": [
                {"id": "test-param-1", "value": 100.0}
            ]
        }
    )
    ...
    tester.test_endpoint(
        "Export GraphML", "GET", f"{BASE_URL}/export/graphml?limit=5"
    )
    
    tester.test_endpoint(
        "Export JSON-LD", "GET", f"{BASE_URL}/export/jsonld?limit=5"
    )
    
    tester.test_endpoint(
        "Export CSV", "GET", f"{BASE_URL}/export/csv?limit=5"
    )
    
    tester.test_endpoint(
        "Export STEP AP242", "GET", f"{BASE_URL}/export/step?limit=5"
    )
    ...
    tester.test_endpoint(
        "Get Versions (404 expected)", "GET", f"{BASE_URL}/version/versions/test-node",
        expected_status=404
    )
    
    tester.test_endpoint(
        "Get History (404 expected)", "GET", f"{BASE_URL}/version/history/test-node",
        expected_status=404
    )
    
    # Test diff endpoint
    tester.test_endpoint(
        "Version Diff (404 expected)", "POST", f"{BASE_URL}/version/diff",
        expected_status=404,
        data={
            "node1_id": "test-node-1",
            "node2_id": "test-node-2"
        }
    )
    
    # Test checkpoint creation
    tester.test_endpoint(
        "Create Checkpoint", "POST", f"{BASE_URL}/version/checkpoint",
        expected_status=201,
        data={
    ...
    tester.test_endpoint(
        "Get Packages", "GET", f"{BASE_URL}/packages"
    )
    
    tester.test_endpoint(
        "Get Classes", "GET", f"{BASE_URL}/classes?limit=5"
    )
    
    tester.test_endpoint(
        "Search", "GET", f"{BASE_URL}/search?q=system"
    )
    
    tester.test_endpoint(
        "Statistics", "GET", f"{BASE_URL}/stats"
    )
