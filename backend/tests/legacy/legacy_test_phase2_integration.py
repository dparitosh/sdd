...
def test_security_features(results: TestResults):
    ...
    try:
        ...
        results.record(
            "Password hashing", len(hashed) > 50 and hashed.startswith("$2b$")
        )
        ...
        limiter = RateLimiter()
        allowed_count = sum(
            1 for _ in range(105) if limiter.is_allowed("test_ip", 100, 60)
        )
...
def test_websocket_handler(results: TestResults):
    ...
    try:
        ...
        results.record("WebSocket connection stats", "active_connections" in stats)
