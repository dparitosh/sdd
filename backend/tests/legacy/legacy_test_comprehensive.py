...
def test_configuration():
    ...
    for var in required_vars:
        ...
        display_value = (
            value
            if var not in ["NEO4J_PASSWORD"]
            else ("*" * 8 if value else "Not set")
        )
...
def performance_benchmark():
    ...
    queries = [
        ...
        (
            "Property keys",
            "CALL db.propertyKeys() YIELD propertyKey RETURN propertyKey LIMIT 10",
        ),
    ]
    ...
    for name, query in queries:
        ...
        print(
            f"  {name:20s}: avg={avg:6.2f}ms  min={min_time:6.2f}ms  max={max_time:6.2f}ms"
        )
