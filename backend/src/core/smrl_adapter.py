...
Canonical location: core/smrl_adapter.py
Provides SMRLAdapter class and convenience functions.
"""
...
class SMRLAdapter:
    ...
    def _format_datetime(cls, dt: Any) -> str:
        ...
        if hasattr(dt, "isoformat"):
            return dt.isoformat()
        # Handle neo4j.time.DateTime / Date / Time objects
        if hasattr(dt, 'year') and hasattr(dt, 'month') and hasattr(dt, 'day'):
            try:
                parts = [f"{dt.year:04d}-{dt.month:02d}-{dt.day:02d}"]
                if hasattr(dt, 'hour'):
                    parts.append(f"{dt.hour:02d}:{dt.minute:02d}:{dt.second:02d}")
                return 'T'.join(parts)
            except Exception:
                pass
        return str(dt)
    ...
    def _get_type_specific_fields(
        ...
        # Datetime-like keys that should be serialized to strings
        datetime_keys = {"created_at", "updated_at", "timestamp", "modified_at",
                         "createdAt", "modifiedAt", "approval_date", "last_seen"}
        ...
        for key, value in node_data.items():
            if (
                ...
                if key in datetime_keys or (value is not None and not isinstance(value, (str, int, float, bool, list, dict))):
                    fields[key] = cls._format_datetime(value)
                else:
                    fields[key] = value
