...
router = APIRouter(prefix="/oslc/client", tags=["OSLC Client"])
...
async def connect_and_discover(request: ConnectRequest):
    ...
    client = OSLCClient(base_url=request.url, auth=auth)
...
async def execute_query(request: QueryRequest):
    ...
    client = OSLCClient(base_url=request.query_capability_url, auth=auth)
    
    try:
        # We manually construct the query call since we're skipping full discovery here
        results = await client.query_resource(
            query_base=request.query_capability_url,
            oslc_where=request.oslc_where
        )
        return results
    except Exception as e:
