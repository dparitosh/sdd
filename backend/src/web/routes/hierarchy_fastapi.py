...
async def get_traceability_matrix(api_key: str = Depends(get_api_key)):
    ...
    try:
        ...
        WITH req, part, direct_owl, sa, COLLECT(DISTINCT mat.name) as materials, COLLECT(DISTINCT owl.name) as ontologies
                ...
                "traceability": [
                    {
                        ...
                        "ontology_id": t.get("ontology_id"),
                        "ontology_name": t.get("ontology_name"),
                        "ontology_type": t.get("ontology_type"),
                        "sa_id": t.get("sa_id"),
                        "sa_name": t.get("sa_name")
                    }
                    for t in r["traceability"]
                    if t.get("part_id") or t.get("ontology_id") or t.get("sa_id")
                ],
            }
            for r in results
        ]
