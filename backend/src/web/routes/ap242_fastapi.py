...
async def get_assemblies(
    ...
    try:
        ...
        query = f"""
        MATCH (asm:Assembly)
        WHERE asm.ap_level = 'AP242' AND {where_clause}
...
async def get_materials(
    ...
    try:
        ...
        query = f"""
        MATCH (mat:Material)
        WHERE mat.ap_level = 'AP242' AND {where_clause}
...
async def get_geometry_models(
    ...
    try:
        ...
        WHERE geo.ap_level = 'AP242' AND {where_clause}
...
async def get_ap242_statistics(api_key: str = Depends(get_api_key)):
    ...
    try:
        ...
        query = """
        MATCH (n)
        WHERE n.ap_level = 'AP242' AND n.ap_schema = 'AP242'
        ...
        return {"ap_level": "AP242", "ap_schema": "AP242", "statistics": stats}
