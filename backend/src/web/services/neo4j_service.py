...
_degraded_mode = False  # Set True when Neo4j was unreachable at startup
...
def get_neo4j_service() -> Neo4jService:
    ...
    Raises ``ServiceUnavailable`` when the application is running in
    **degraded mode** (Neo4j was unreachable at startup).

    Returns:
        Neo4jService: Singleton service instance

    Raises:
        ServiceUnavailable: when Neo4j is unreachable (degraded mode)
    """
    global _neo4j_service

    # Fast exit when running in degraded mode (Neo4j down at startup)
    if _degraded_mode:
        raise ServiceUnavailable(
