...
Migration note
~~~~~~~~~~~~~~
The canonical config / driver live in ``src.core.config`` and
``src.core.database``.  During the migration the container still
instantiates ``Neo4jService`` (which reads its own env vars), but
new services should use ``get_settings()`` and ``get_driver()`` from
``src.core`` instead.
"""
...
class ServiceContainer:
    ...
    def startup(self) -> None:
        ...
        If Neo4j is unreachable the container still starts in degraded mode
        ...
        try:
            ...
            logger.error(f"ServiceContainer: Neo4j unavailable – {exc}")
            # Keep neo4j_service as None so callers can detect degraded mode
            self.neo4j_service = None
            self._started = True

            # Patch the legacy singleton module so get_neo4j_service()
            # returns None immediately instead of retrying for 12 s.
            import src.web.services.neo4j_service as _ns
            _ns._degraded_mode = True

            raise  # let lifespan decide whether to crash or continue
...
class Services:
    ...
    def neo4j():
        ...
        Raises ``ServiceUnavailable`` when the app is in degraded mode
        (Neo4j was unreachable at startup).  The global exception handler
        in ``app_fastapi.py`` converts this to a 503 response.
        """
        from neo4j.exceptions import ServiceUnavailable

        svc = ServiceContainer.instance().neo4j_service
        if svc is None:
            raise ServiceUnavailable(
                "Neo4j service unavailable (application in degraded mode)"
            )
        return svc
