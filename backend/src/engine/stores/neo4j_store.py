...
class Neo4jGraphStore:
    ...
    def __init__(
        ...
        # Prefer explicit args, then core Settings, then env vars.
        try:
            from src.core.config import get_settings
            _s = get_settings()
            _uri = uri or _s.neo4j_uri
            _user = user or _s.neo4j_user
            _password = password or _s.neo4j_password
            _database = database or _s.neo4j_database
        except Exception:
            # Graceful fallback if core not bootstrapped yet
